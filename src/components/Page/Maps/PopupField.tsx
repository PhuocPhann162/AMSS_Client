...
import { findNearestRiver, getPlantSuggest, getScrollAnimation } from '~/helper';
import { cropModel, fieldModel, plantSuggestModel } from '~/interfaces';
...
export const PopupField = ({ fieldInfo }: PopupFieldProps) => {
  ...
  const [cropDetail, setCropDetail] = useState<cropModel | null>(null);
  // const [platnSuggest, setPlantSuggest] = useState<plantSuggestModel | null>(null);
  const [deleteField] = useDeleteFieldMutation();
  const { data, isLoading } = useGetCropByIdQuery(fieldInfo.fieldCrops![0].cropId);
      ...
      {!isLoading && (
        <div className='flex flex-col w-72 gap-1 '>
          ...
          <div className='flex items-center justify-between'>
            ...
            <div>
              <NavLink to={`/app/land/field/suggestion/plantDetail`} className='flex items-center gap-2'>
                <motion.img
