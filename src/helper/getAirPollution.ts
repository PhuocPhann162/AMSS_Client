...
export const evaluatePollutantLevelDescription = (
  pollutant: string,
  value: number,
): string => {
  ...
  if (value < thresholds[pollutant].good) {
    ...
    value >= thresholds[pollutant].good &&
    value < thresholds[pollutant].fair
  ) {
    return 'Moderate level of pollution';
  } else if (
    value >= thresholds[pollutant].fair &&
    value < thresholds[pollutant].moderate
  ) {
    return 'High level of pollution';
  } else if (
    value >= thresholds[pollutant].moderate &&
    value < thresholds[pollutant].poor
  ) {
